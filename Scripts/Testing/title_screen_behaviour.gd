extends CanvasLayer
class_name Title_Scene_Behaviour

func on_level1_button_pressed():
	get_tree().change_scene_to_file("res://Scenes/Levels_&_Screens/nivel_1.tscn")
 
func on_level2_button_pressed():
	get_tree().change_scene_to_file("res://Scenes/Levels_&_Screens/nivel_3.tscn")

func on_level3_button_pressed():
	get_tree().change_scene_to_file("res://Scenes/Elements/hability_3_spawner.tscn")

func on_level4_button_pressed():
	get_tree().change_scene_to_file("res://Scenes/Levels_&_Screens/nivel_4.tscn")
